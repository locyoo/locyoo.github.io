...
举一个栗子，在第33行。`<a target="_blank" href="https://www.zhihu.com/people/{{ site.zhihu_username }}">`<br>
`target="_blank"`这段表示后边的链接会在新的标签页打开<br>
让我们看看知乎个人主页的链接形式，拿我的举例，`https://www.zhihu.com/people/luminouszhu-jiang-wan`他的形式是`https://www.zhihu.com/people/`+`最开始创建用户的名字`（但是这个后边加的这段是可以[更改的，但只能更改一次](https://www.zhihu.com/settings/account)。<br>
看代码，`{{ site.zhihu_username }}">`看后边的`zhihu_username`，是不是很眼熟？对，你在`_config.yml`里见过，它在第17行，在`_config`里，`zhihu_username`后面接的就是你个人主页的后边那串，我的就是`luminouszhu-jiang-wan`。那我`_config.yml`第17行`zhihu_username`后面就应该写`luminouszhu-jiang-wan`，而我的用户名实际是`CheongSzesuen`所以这个得看实际情况。<br>
所以我推测可以继续加社交网站，暂时不知道怎么设置图标，待我学qiubaiying先生加的简书归来后我会继续写。（挖坑）
##### 3. _layout文件夹下的
###### （1）搜索引擎搜出来显示的图标
